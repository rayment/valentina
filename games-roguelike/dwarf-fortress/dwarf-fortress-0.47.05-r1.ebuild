...
QA_PREBUILT="usr/games/${PN}/libs/Dwarf_Fortress"
...
src_install() {
	insinto /usr/games/${PN}
	doins -r data libs raw

	fperms +x /usr/games/${PN}/libs/Dwarf_Fortress
	...
	local DOC_CONTENTS="
		Dwarf Fortress has been installed to ${EPREFIX}/usr/games/${PN}. This is
		symlinked to ~/.config/${PN} when ${PN} is run. For more information on
		what exactly is replaced, see ${EPREFIX}/usr/bin/${PN}. Note: This means
		that the primary entry point is ${EPREFIX}/usr/bin/${PN}, do not run
		${EPREFIX}/usr/games/${PN}/libs/Dwarf_Fortress."
